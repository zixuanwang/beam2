...
void exit_with_help() {
	std::cout << "Usage: xbox_vad input_file output_file\n";
	exit(1);
}
...
int main(int argc, char* argv[]) {
	...
	while (true) {
		...
		vad.process(fft_input);
		out_stream << time << "\t" << time + (double)FRAME_SIZE / (double)SAMPLE_RATE << "\t" << vad.GetSpeechPresenceProb() << std::endl;
